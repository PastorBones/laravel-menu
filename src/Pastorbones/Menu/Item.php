	...
	*/
	...
	public function activate( \Lavary\Menu\Item $item = null, $isParent = false ){
		...
		$item = is_null($item) ? $this : $item;
                ...
                $active_element_parent = $this->builder->conf('active_element_parent');

                // Check to see which element should have class 'active' set.
                if($isParent)
                {
                    if($active_element_parent == 'item' || $active_element_parent == 'both')
                    {
                        $item->active();
                    }
                    if($active_element_parent == 'link' || $active_element_parent == 'both')
                    {
                        $item->link->active();
                    }
                } else
                {
                    if($active_element == 'item' || $active_element == 'both')
                    {
                        $item->active();
                    }
                    if($active_element == 'link' || $active_element == 'both')
                    {
                        $item->link->active();
                    }
                }
